...
int main() {
    ...
    int nb_commandes = 0; // Variable pour stocker le nombre de commandes
    int distance[20] = {0}; // Tableau pour stocker les commandes  distances
    int angle[20] = {0}; // Tableau pour stocker les commandes  angles
    float x_robot[400] = {0}; // Tableau pour stocker les coordonnées x du robot
    float y_robot[400] = {0}; // Tableau pour stocker les coordonnées y du robot
    ...
    FILE *position_robot = NULL;  // Fichier pour stocker la position du robot
    ...
    while (1) {
        ...
        Robot_Commandes(message, &nb_commandes, distance, angle);//extraire les commandes du message saisie sur le terminal
        Position_Robot(x_robot, y_robot, nb_commandes, distance, angle);// calculer les coordonnes du robot 

        // Stocker les coordonnées du robot dans le fichier position_robot.txt
        position_robot = fopen("position_robot.txt", "w");
        for (i = 0; i < (nb_commandes*20) + 1; i++) {
            //printf("position %d : X = %.2f\tY = %.2f\n",i, x_robot[i], y_robot[i]);
            fprintf(position_robot, "%.2f\t%.2f\n", x_robot[i], y_robot[i]);
        }
        fclose(position_robot);
